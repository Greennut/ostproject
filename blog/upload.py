#!/usr/bin/env python

import os
import urllib
import webapp2

from google.appengine.api import users
from google.appengine.ext import blobstore, db, webapp
from google.appengine.ext.webapp import blobstore_handlers, template
from google.appengine.runtime.apiproxy_errors import CapabilityDisabledError
import json
import logging
import re

class Image(db.Model):
    user = db.UserProperty(auto_current_user=True)
    blob = blobstore.BlobReferenceProperty(required=True)
    date = db.DateTimeProperty(auto_now_add=True)


class UploadHandler(blobstore_handlers.BlobstoreUploadHandler):
    def post(self):
        try:
            upload_files = self.get_uploads('file')
            if len(upload_files) > 0:
                blob_info = upload_files[0]
                Image(blob=blob_info.key()).put()

                pattern = re.compile("https?://(.*?)/")
                match = pattern.match(self.request.url)
                if match is not None:
                    url_base = match.group(0)
                else:
                    url_base = "/"
                self.response.out.write(json.dumps({"success": True, "url": url_base + "serve/%s" % blob_info.key()}))
            else:
                self.response.out.write(json.dumps({"success": False}))
        except CapabilityDisabledError:
            self.response.out.write('Uploading disabled')


class ServeHandler(blobstore_handlers.BlobstoreDownloadHandler):
    def get(self, resource):
        resource = str(urllib.unquote(resource))
        blob_info = blobstore.BlobInfo.get(resource)
        self.send_blob(blob_info)


class DeleteHandler(webapp2.RequestHandler):
    def post(self):
        try:
            key = self.request.get("key")
            wrapper = Image.get(key)
            if wrapper:
                if wrapper.blob:
                    blobstore.delete(wrapper.blob.key())
                else:
                    self.response.out.write('No blob in wrapper')
                db.delete(wrapper)
                self.redirect('/')
            else:
                self.response.out.write('No wrapper for key %s' % key)
        except CapabilityDisabledError:
            self.response.out.write('Deleting disabled')


class MainHandler(webapp2.RequestHandler):
    def get(self):
        if users.get_current_user():
            loginout_url = users.create_logout_url('/')
        else:
            loginout_url = users.create_login_url('/')
        values = {
            'user': users.get_current_user(),
            'users': users,
            'upload_url': blobstore.create_upload_url('/upload/upload'),
            'wrappers': Image.all(),
        }
        path = os.path.join(os.path.dirname(__file__), 'main.html')
        self.response.out.write(template.render(path, values))


class AccountHandler(webapp2.RequestHandler):
    def get(self):
        if users.get_current_user():
            self.redirect(users.create_logout_url('/'))
        else:
            self.redirect(users.create_login_url('/'))


app = webapp2.WSGIApplication([
                                  ('/upload/', MainHandler),
                                  ('/upload/account', AccountHandler),
                                  ('/upload/upload', UploadHandler),
                                  ('/serve/([^/]+)?', ServeHandler),
                                  ('/upload/delete', DeleteHandler),
                              ], debug=True)
